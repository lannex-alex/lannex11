...
class AirwallexAccount(models.Model):
    ...
    oauth2_client_id = fields.Char('OAuth2 Client ID', help="The ID of your application issued by Airwallex")
    oauth2_secret = fields.Char('OAuth2 Secret', help="The secret of your application issued by Airwallex")
    ...
    def _get_redirect_url(self):
        ...
        return 'https://awx15.dev.portcities.cc' + OAUTH2_CALLBACK_ROUTE
    ...
    def refresh_oauth2_token(self):
        ...
        generate_code_response = self.call_airwallex_api(OAUTH2_TOKEN_ENDPOINT, params=data, is_authentification='oauth2')
    ...
    def auth_api(self):
        if not self.client_id or not self.api_key:
            raise UserError(_("Please enter a client ID and an API key to test the authentification"))
        response = self.call_airwallex_api(AUTHENTICATION_ENDPOINT, payload="{}", is_authentification='api')
    ...
    def call_airwallex_api(
            ...
            is_authentification=False, force_new=False, headers=False, params=False
    ):
        self.ensure_one()
        base_url = self._get_file_url() if is_file else self._get_url()
        if is_authentification != 'oauth2':
        ...
        if is_authentification == 'api':
            headers.update({
                "x-api-key": self.api_key,
                "x-client-id": self.client_id
            })
        elif not is_authentification:
            headers.update({
